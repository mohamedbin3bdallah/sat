...
function handleChange(id,type,checkbox) {
    if(checkbox.checked == true){
		window.location.href = '{{url("admin/services/products/property")}}'+'/'+id+'/'+type;
    }else{
        window.location.href = '{{url("admin/services/products/property")}}'+'/'+id+'/'+0;
   }
}
</script>
...
<div class="main">
  ...
  @if (session('Message_pro_success'))
	<ul class="ul-success">
		<li class="li-success">
			{{session('Message_pro_success')}}
		</li>
	</ul>
  @endif
  @if (session('Message_pro_fail'))
	<ul class="ul-danger">
		<li class="li-danger">
			{{session('Message_pro_fail')}}
		</li>
	</ul>
  @endif
    ...
    <tbody>
      @foreach ($alldata as $key => $data)
        <tr>
		  ...
		  <td><input type="checkbox" value="{{($data->flag_view==1)? 0:1}}" onchange="handleChange({{$data->id}},1,this);" {{($data->flag_view==1)? 'checked':''}} /></td>
		  <td><input type="checkbox" value="{{($data->flag_view==2)? 0:2}}" onchange="handleChange({{$data->id}},2,this);" {{($data->flag_view==2)? 'checked':''}} /></td>
          <td><a href="{{url('admin/servicedetails/product/'.str_replace(' ','_',$data->title))}}"><button type="button" class="btn btn-default" ><span class="glyphicon glyphicon-eye-open"></span></button></a></td>
		  <td><a class="btn btn-default" href="{{url('admin/services/products/delete/'.str_replace(' ','_',$data->title))}}"><span class="glyphicon glyphicon-remove"></span></a></td>
        </tr>
      @endforeach
    </tbody>
  </table>
