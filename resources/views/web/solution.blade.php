...
<link rel="stylesheet" type="text/css" href="{{asset('resources/assets/styles/solution.css')}}">
<link rel="stylesheet" type="text/css" href="{{asset('resources/assets/styles/solution-responsive.css')}}">

<style>
.img_h {width: auto; height: 100%;}
.img_w {width: 100%; height: auto;}
.carousel-item {width: 352px; height: 285px;}
.solution_col {border: 2px solid #0d50a2; border-radius: 15px;}
.center {
  display: block;
  margin: auto;
}
...
<div id="carousel-thumb" class="carousel slide carousel-fade carousel-thumbnails" data-ride="carousel">
	...
	<?php $keys = array(); ?>
	@foreach ($subcategory->solution as $p_key => $solution)
	@if(!empty($solution->image))
	@foreach ($solution->image as $img_key => $image)
	<?php $keys[] = $p_key.$img_key; ?>
        <div class="carousel-item {{ ($p_key == 0 and $img_key == 0)? 'active':'' }}">
			...
			<a href="{{url('solution_details/'.$solution->id)}}"><figcaption><div><h4 style="background-color:#fff;">{{$solution->title}}</h4></div></figcaption></a>
        </div>
	@endforeach
	@endif
	@endforeach
...
@endif
                        ...
                        <div class="features_item_content">
							<div class="features_item_title"><a href="{{url('solution/'.$subcategory->id)}}">{{$subcategory->title}}</a></div>
...
<!--           solution          -->
				@if (!empty($subcategory->solution()))
					<?php $keys_2 = array(); ?>
          <div class="solution ">
            <div class="row solution_row">
     <!--    Info Item                  -->
		 					@foreach ($subcategory->solution as $key_2 => $service)
							<?php $keys_2[] = $key_2; ?>
                    <div class="col-lg-4 solution_col">
              ...
              <a href="{{url('solution_details/'.$service->id)}}"></a>
            </figure>
